...
The companion `tools/verify_captive_orbit_dispatch.sh` gate proves only that
raw scan `47` was delivered through the emulated AT controller and the live
CAPPO bridge. It does not claim that a fixed code address consumed the byte;
the older `0824:06B8` breakpoint assumption was removed after failing to match
the running CAPPO relocation.
...
```sh
tools/verify_captive_live_replay.sh /path/to/original/captive [BUILD_DIR]
```

This sends the original Orbit, Land, Forward, turn, and Forward scan codes
through the live CAPPO IRQ1 queue. It waits for a complete dump after each
observation, then requires the dump, DOS-VGA frame, and runtime frame
comparison to pass. The gate also requires multiple authentic frame changes.
It is local-only because the original media is supplied by the player; no
map, position, graphics, or text is generated by the helper. A new dump is
not treated as a semantic CAPPO acknowledgement: orbit, arrival, landing,
and dungeon entry still require their own authentic runtime evidence.

For timing diagnostics, the optional live FIFO also accepts `WAIT:<ticks>`.
This advances only the original CAPPO timer (1–2000 ticks) and injects no
