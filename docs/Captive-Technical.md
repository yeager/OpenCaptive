...
through the live CAPPO IRQ1 queue. It waits for CAPPO's own acknowledgement
before copying each dump, then requires the dump, DOS-VGA frame, and runtime
frame comparison to pass. The gate also requires multiple authentic frame
changes. It is local-only because the original media is supplied by the
player; no map, position, graphics, or text is generated by the helper.
