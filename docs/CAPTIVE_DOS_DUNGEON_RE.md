...
## RESOLVED (2026-08-20): the level generator is GM.EXE, run headless

The open question above ("find the routine that FILLS 0x7CB3") is answered, and the
"multi-session DOS-overlay wall" is no longer the blocker for level generation.
CAPPO.EXE is only the front-end/menu; the actual dungeon LEVEL is generated by a
**separate child program, GM.EXE**, which CAPPO launches via **INT 21h AH=4Bh
(EXEC)** from its level-setup path. This is why CAPPO alone never produced a level:
with EXEC stubbed in the harness, GM.EXE never ran, so every state-1->3 transition
bounced straight back to the menu.

Chain (CAPPO_CODE.bin offsets, CS=0; DGROUP para 0x0E3F):
- mode init `0x4284` (dispatched with the per-tick handler `0x44CE` from ~0x0E88)
  sets game state = 1 and runs the setup chain;
- per-state setup `0xC9D9` -> `call 0x10DBC` (near-wrap = `0x0DBC`) = LEVEL SETUP;
- `0x0DBC` builds a DOS EXEC parameter block at `0000:04F0` (mission params ax/cx/
  bx/dx at +4/+6/+8/+A, DGROUP at +2) and INT 21 AH=4Bh on the program name at
  `DGROUP:0x08` = **"GM.EXE"** (bytes 47 4D 2E 45 58 45 00);
- the state-1->3 transition `0x4520` (fires when party==cursor and phase!=4) is
  verified reached; it only "sticks" once GM.EXE has laid down a real level.

GM.EXE (LZEXE-packed 10541b -> `unlzexe` -> GM_UNP.EXE 40408b). It makes NO file/
DOS calls except exit (AH=4Ch): it is a pure-computation generator that reads the
mission params from fixed low memory (`0000:04F4..04FA`) and writes the generated
map directly into memory via absolute segments (DOS has no memory protection).
Generation pipeline (GM_UNP.EXE offsets, verified by emulation):
- `0x2F2`  seed/init from the mission parameter (word[0x3078]);
- core placement state machine at `0x1F00`+ (writers 0x1F2B/0x05E7/0x26B5) FILLS the
  input map (words at 0x2CF:0x38) — dispatches on cell type + counters to room/
  corridor/feature placers (0x1F95/0x1F8D/0x24A9/0x2055/...);
- `0xEE`   the 2048-cell generate+translate loop: for each cell it calls the code
  translator `0x129` and writes the result to the output map at `0x2CF:0x5A68`
  (di=word[0x3578]); a bounded loop that returns cleanly;
- copy loop `0xCD..0xE7` distributes the finished tables back into CAPPO's DGROUP
  via the `0000:04F0` far pointer (incl. the 64x32 map at DG:0x7CB3), then GM exits.

Verified end-to-end in the Unicorn harness (opencaptive-re/, real game files only):
- `emu_drive2.py` now implements INT 21 AH=4Bh: on EXEC it loads GM_UNP.EXE above
  CAPPO in shared memory and runs it; GM's writes correctly target CAPPO's map.
- `gmbuf.py`/`gmclean.py` run GM standalone with a mission param, stop at gen_ret
  (0x128) and read the clean 64x32 map at 0x2CF:0x5A68 — producing DISTINCT real
  base layouts per mission (ax=1/2/3 differ), with clear rooms and corridors.

Output cell codes (GM display format, from the `0x129` translator): 0x00 = empty/
outside, 0x20 = floor/default, 0xA0 = wall (translation of source 0x20), plus door/
feature tiles.  The wall test on the copied-back map is CAPPO's holomap semantic
`(code & 0x7F) > 0x1A`.

### Transcribed to the codebase
`0x129` (the per-cell code translator) is transcribed byte-for-byte into
`src/data/captive_gm_translate.c` (`captive_gm_translate_cell`) and verified in
`tests/test_captive_gm_translate.c` against the 256-entry ground-truth table
captured from the real GM.EXE (opencaptive-re/xlate_ref.py), plus the dh/ax/marker
special cases.  Remaining for a fully-native generator: transcribe the `0x1F00`
