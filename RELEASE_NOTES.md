...
- **(Captive)** Original CAPPO holomap controls for keypad movement, Orbit,
  Land, and Enter-based cursor centering are documented and covered by the
  native regression tests.
- **(Infrastructure)** Release packaging checks now include Captive reference
  assets on Windows and require the checked-in SVG icon for iOS and Android.

### Changed

- **(Captive)** Mission 0001 starts on the verified real holomap reference
  frame without a synthetic whole-frame overlay. The live mission/runtime
  decoder remains explicitly gated until its original records are decoded.
- **(Documentation)** The README, public documentation, and wiki are aligned
  with v1.1.101, the 62-test suite, cached data scanning, version selection,
  live F10 options, and the source-backed Captive controls.

### Removed

- **(Infrastructure)** Synthetic Android icon fallback generation from the
  release workflow.
