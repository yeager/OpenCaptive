...
- The shop menu now shows item names together with price and selection state.
- The release check verifies Captive data against both DOS and Amiga sources.

### Changed

- `creature_killed` and `level_up_occurred` are now reported per attack and
  cannot leak from a previous shot.
...
- Spray weapons use range 4, while other ranged weapons use range 6.
- Seeded Captive saves now restore their authoritative mission seed and can
  continue floor transitions correctly after loading.
- The start menu synchronizes version and platform selection after rescanning,
  and new launcher and droid strings are localized in all 19 languages.
- The release workflow now requires the requested release version to match the
  CMake version before artifacts are published.
...
- Captive scanning, saves, puzzle progression, viewport, and combat now have
  broader deterministic regression coverage across multiple seeds.
- Full local verification covers 58/58 CTest tests.
- GitHub Actions builds and tests macOS, Linux, and Windows before the release
  job is allowed to create the GitHub release.

## v1.1.79 (2026-08-06)

### Added

- The F10 menu now provides direct control over visual enhancements, filters,
  scanlines, CRT, lighting, help overlays, and optional cheats.
- Replay recordings can be saved on exit with `--replay-record`; the path can
  be selected with `--replay-output`.
- Linux packages, RPM, and AppImage now ship the resources required by the
  start menu and find them after installation.
- The release workflow can be started manually with an explicit semantic
  version and creates the corresponding GitHub tag.

### Changed

- Saves, cross-save, replay, and configuration are written atomically so an
  interruption cannot destroy the last working file.
- Captive saves use a portable little-endian format, while older versions
  remain readable.
- `--hd-upscale`, `--upscale-factor`, `--widescreen`, and `--hq-midi` now have
  an observable effect during gameplay.
- Liberation correctly restores transient gameplay state on Continue and
  rebuilds the seeded city before restoring state.
- The data cache and ZIP search are more robust when archives are replaced,
  files change rapidly, or compressed entries are empty.
- Release builds verify that all desktop and mobile packages exist before a
...
- The Architect map generator now uses an incremental frontier and avoids
  repeated full-map scans. The broad seed regression is now fast enough for CI.
...
- Linux packages and AppImages now find fonts, localization, and launcher
  images through `../share/opencaptive` and `/usr/share/opencaptive`, not only
  through the development directory beside the binary.
- Linux RPM and AppImage packages now include fonts, all localization
  catalogs, and menu images, giving installed packages the same launcher
  resources as the tarball, DEB, and Windows installer.
- VFS cache validation now uses a metadata probe without reading file contents
  for every hash lookup. It also detects replaced loose files while reusing
  already verified content from the cache.
- Liberation F9 now rebuilds the seeded city before restoring saved state, so a
  mission from another session cannot be mixed with old buildings and cells.
- Windows VFS cache metadata now uses high-resolution filesystem timestamps and
  file identity, preventing rapid ZIP replacements from reusing stale entries.
- Captive saves now use an explicit little-endian format in version 5. New
  files are independent of C structure padding and enum size and can move
  between macOS, Windows, and Linux; older v3/v4 files remain readable.
- A cache signature is recomputed for every hash lookup. Replacing a ZIP while
  the same VFS instance is alive can no longer reuse stale cache entries.
- VFS cache data and metadata are written to process-unique temporary files and
  replaced atomically, so parallel starts and interruptions cannot leave a
  partially written cache pair.
- Save/load and cross-save errors are now reported clearly on stderr instead of
  being silently ignored by the input flow.
- `--replay-record` now writes the replay file on exit, using
  `opencaptive.ocrp` by default. `--replay-output <file>` selects another path.
- Replay files and feature configuration are also written atomically, so an
  interrupted recording or configuration write cannot leave a truncated file.
- Captive portable cross-save exports are written to a temporary file and
  replaced atomically, so a failed export cannot destroy an existing `.ocsv`.
- Liberation saves use the same atomic replacement as Captive; failed writes
  leave the previous working save intact.
- Captive saves are written to a temporary file and replaced atomically in
  place, preserving the last working save after an interrupted write.
- `hq_midi` is now active: `--hq-midi` applies a short output filter after OPL2
  synthesis. Default mode and timing are unchanged.
- `--widescreen` is now connected to presentation and expands the native image
  to 16:9 (or the selected `widescreen_width`) without changing the verified
  native framebuffer.
- `--hd-upscale` and `--upscale-factor` now use xBRZ in the actual framebuffer
  presentation for Captive and Liberation; previously they only changed
  configuration.
- The start menu's audio sample rate is now effective: 22,050, 44,100, and
  48,000 Hz are used by the SDL audio stream, SFX resampling, and MIDI timing.
- Captive F5 saves also create the portable `.ocsv` file when cross-save export
  is enabled; quicksave keeps the same slot number.
- Liberation sessions now reset transient dungeon, building, combat, and
  inventory state. An invalid Continue save starts a clean session instead of
  inheriting state from the previous game.
- Puzzle solution hints now use localization, including solution, matching,
  code, and password labels.
- The VFS now reads valid empty deflated ZIP entries correctly, preventing an
  empty file in a compressed archive from being reported as missing.
- Visible screen headings and status messages now use localization, and the
  POT plus all 18 translated catalogs are updated. English is the fallback for
  strings that do not yet have a local translation.
- Architect base maps now have a minimum playable density per logical floor,
  preventing seeds such as 161 from creating an undersized root floor.
- Liberation map finalization now uses the correct cell plane for navigation
  and rendering while keeping building IDs separate for entrances, dialogue,
  wall colors, and fast travel.
- ZIP-based game data is now discovered in subdirectories below the selected
  data root, making recursive scanning consistent for loose files and archives.
- Puzzle records are now reset when the puzzle generator list is reused, so
  stale target coordinates cannot leak into new unlinked power-socket puzzles.
...
- CustomFeatures configuration now accepts optional whitespace around the
  equals sign.
...
- Windows VFS cache signatures are now deterministic because directory metadata
  is sorted before hashing, so unchanged game data is not rescanned due to
