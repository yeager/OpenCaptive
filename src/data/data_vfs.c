...
static void vfs_make_parent_dirs(const char *filepath) {
    char tmp[1024];
    size_t n = strlen(filepath);
    if (n == 0 || n >= sizeof(tmp)) return;
    memcpy(tmp, filepath, n + 1);
    for (char *p = tmp + 1; *p; p++) {
        if (*p == '/') { *p = '\0'; (void)vfs_mkdir(tmp); *p = '/'; }
    }
}

/* The DOS runtime (DOSBox) mounts a real directory, but the Captive DOS game
 * ships inside a ZIP.  Extract the archive that carries `marker` (e.g.
 * "CAPTIVE.BAT") into dest_dir, preserving entry paths, and report the directory
 * that directly contains the marker so the caller mounts the right level. */
bool vfs_extract_archive_tree(const DataVFS *vfs, const char *marker,
    ...
    if (!vfs || !vfs->initialized || !marker || !dest_dir) return false;

    const char *zip_path = NULL;
    for (int i = 0; i < vfs->num_zips; i++) {
        size_t sz = 0;
        uint8_t *probe = zip_extract(vfs->zip_paths[i], marker, &sz);
        if (probe) { free(probe); zip_path = vfs->zip_paths[i]; break; }
    }
    ...
    long zip_size = ftell(f);
    if (zip_size < 22) { fclose(f); return false; }
    long search_start = zip_size - 22;
    if (search_start > zip_size - 65557) search_start = zip_size - 65557;
    if (search_start < 0) search_start = 0;
    long search_len = zip_size - search_start;
    uint8_t *eocd_buf = malloc((size_t)search_len);
    if (!eocd_buf) { fclose(f); return false; }
    if (fseek(f, search_start, SEEK_SET) != 0 ||
        fread(eocd_buf, 1, (size_t)search_len, f) != (size_t)search_len) {
        free(eocd_buf); fclose(f); return false;
    }
    long eocd_off = -1;
    for (long i = search_len - 22; i >= 0; i--)
        if (read_u32(eocd_buf + i) == ZIP_END_SIG) { eocd_off = search_start + i; break; }
    free(eocd_buf);
    if (eocd_off < 0) { fclose(f); return false; }
    if (fseek(f, eocd_off, SEEK_SET) != 0) { fclose(f); return false; }
    uint8_t eocd[22];
    if (fread(eocd, 1, 22, f) != 22) { fclose(f); return false; }
    uint16_t num_entries = read_u16(eocd + 10);
    uint32_t cd_offset = read_u32(eocd + 16);
    if (fseek(f, cd_offset, SEEK_SET) != 0) { fclose(f); return false; }

    (void)vfs_mkdir(dest_dir);
    bool any = false;
    char marker_dir[1024]; marker_dir[0] = '\0';
    ...
    for (uint16_t e = 0; e < num_entries; e++) {
        ...
        uint16_t extra_len = read_u16(cd_hdr + 30);
        uint16_t comment_len = read_u16(cd_hdr + 32);
        if (name_len == 0 || name_len > ZIP_MAX_ENTRY_NAME) break;
        char name[ZIP_MAX_ENTRY_NAME + 1];
        if (fread(name, 1, name_len, f) != name_len) break;
        name[name_len] = '\0';
        if (fseek(f, extra_len + comment_len, SEEK_CUR) != 0) break;
        if (name[name_len - 1] == '/') continue;           /* directory entry */
        if (name[0] == '/' || strstr(name, "..")) continue; /* path safety */

        long saved = ftell(f);
        size_t sz = 0;
        uint8_t *data = zip_extract(zip_path, name, &sz);
        if (data) {
            char outpath[1024];
            if (snprintf(outpath, sizeof(outpath), "%s/%s", dest_dir, name)
                    < (int)sizeof(outpath)) {
                for (char *p = outpath; *p; p++) if (*p == '\\') *p = '/';
                vfs_make_parent_dirs(outpath);
                FILE *of = fopen(outpath, "wb");
                if (of) {
                    if (sz == 0 || fwrite(data, 1, sz, of) == sz) any = true;
                    fclose(of);
                    /* Record the directory holding the marker, for mounting. */
                    int nlen = (int)strlen(name);
                    if (marker_dir[0] == '\0' && nlen >= mlen &&
                        (nlen == mlen || name[nlen - mlen - 1] == '/' ||
                         name[nlen - mlen - 1] == '\\')) {
                        const char *suf = name + nlen - mlen;
                        bool match = true;
                        for (int k = 0; match && k < mlen; k++) {
                            char a = suf[k], b = marker[k];
                            if (a >= 'a' && a <= 'z') a -= 32;
                            if (b >= 'a' && b <= 'z') b -= 32;
                            if (a == '\\') a = '/'; if (b == '\\') b = '/';
                            if (a != b) match = false;
                        ...
                        if (match) {
                            snprintf(marker_dir, sizeof(marker_dir), "%s", outpath);
        ...
        if (saved < 0 || fseek(f, saved, SEEK_SET) != 0) break;
    }
    fclose(f);
    if (!any) return false;
    if (mount_dir_out && mount_dir_size)
        snprintf(mount_dir_out, mount_dir_size, "%s",
                 marker_dir[0] ? marker_dir : dest_dir);
    return true;
}
