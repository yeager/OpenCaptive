...
int main(int argc, char *argv[]) {
    ...
    bool captive_dungeon_requested = false;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            printf(
                ...
                "  --captive-dungeon    Enter a native Captive dungeon generated by the GM.EXE port\n"
        ...
        } else if (strcmp(argv[i], "--captive-dungeon") == 0) {
            requested_game = GAME_CAPTIVE;
            start_directly = true;
            captive_dungeon_requested = true;
    ...
    if (start_directly && requested_game == GAME_CAPTIVE) {
        if (!validate_data_path(&vfs)) {
            ...
            if (captive_dungeon_requested) {
                /* Native dungeon entry: generate the level with the byte-exact
                 * GM.EXE port and drop straight into the first-person viewport. */
                if (!game_state_new_captive_mission(&gs, gs.mission)) {
                    fprintf(stderr, "Captive dungeon generation failed\n");
                    capture_failed = capture_frame_path != NULL;
                } else {
                    combat_init(&creatures);
                    puzzle_init(&puzzles);
    ...
    while (running) {
        ...
        switch (gs.mode) {
            ...
            case STATE_GAME: {
                ...
                if (gs.game_type == GAME_CAPTIVE && captive_dos_memory_active) {
                ...
                } else if (gs.game_type == GAME_CAPTIVE && gs.num_levels > 0) {
                    /* Native Captive dungeon: the level was generated by the
                     * byte-exact GM.EXE port (game_state_new_captive_mission);
                     * render its first-person viewport with the verified PL5
                     * panel sheets — no DOSBox, no synthetic map. */
                    if (hud_bg)
                        memcpy(framebuffer, hud_bg,
                               CAPTIVE_ORIGINAL_WIDTH * CAPTIVE_ORIGINAL_HEIGHT *
                               sizeof(uint32_t));
                    ...
                    if (config.render_mode == CAPTIVE_RENDER_ORIGINAL)
                        viewport_render_original_descriptors(
                            &vw, &atlas, framebuffer,
                            CAPTIVE_ORIGINAL_WIDTH, CAPTIVE_ORIGINAL_HEIGHT);
                    else
