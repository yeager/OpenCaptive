...
static void test_combat_does_not_spend_energy_for_invalid_item_database(void) {
    GameState gs;
    CreatureList creatures = {0};
    ItemDatabase db = {0};
    game_state_init(&gs, GAME_CAPTIVE, 1);
    ...
    int energy_before = gs.droids[0].energy;
    assert(!combat_droid_attack_with_items(&gs, &creatures, 0, &db));
    assert(gs.droids[0].energy == energy_before);
    assert(creatures.creatures[0].hp == 30);
}

static void test_combat_rejects_weapon_id_with_wrong_item_category(void) {
    GameState gs;
    CreatureList creatures = {0};
    ItemDatabase db;
    item_db_init(&db);
    ...
    for (int i = 0; i < db.num_defs; i++) {
        if (db.defs[i].id == 18) {
            db.defs[i].category = ITEM_ARMOR_HEAD;
            break;
        }
    }
    creatures.num_creatures = 1;
...
static void test_grenade_rejects_id_with_wrong_item_category(void) {
    GameState gs;
    CreatureList creatures = {0};
    ItemDatabase db;
    item_db_init(&db);
    ...
    for (int i = 0; i < db.num_defs; i++) {
        if (db.defs[i].id == 60) {
            db.defs[i].category = ITEM_EXPLOSIVE;
            break;
        }
    }
    creatures.num_creatures = 1;
...
static void test_combat_shield_absorbs_before_body_damage(void) {
    GameState gs;
    CreatureList creatures = {0};
    game_state_init(&gs, GAME_CAPTIVE, 1);
    game_state_new_mission(&gs, 1);
    ...
    /* Isolate one living droid so the target choice is deterministic. */
    for (int i = 1; i < 4; ++i) gs.droids[i].hp = 0;
    gs.droids[0].shield = 58;
    gs.droids[0].shield_hp = 10;
    memset(gs.droids[0].body_part_hp, 255, sizeof(gs.droids[0].body_part_hp));
    uint8_t body_part_hp[6];
    memcpy(body_part_hp, gs.droids[0].body_part_hp, sizeof(body_part_hp));

    creatures.num_creatures = 1;
    creatures.creatures[0] = (Creature){
        .type = CREATURE_ALIEN1, .hp = 20, .hp_max = 20,
        .damage_min = 10, .damage_max = 10, .range = 1, .speed = 0,
        .x = 2, .y = 1, .level = 0, .active = true, .alerted = true,
    };
    int hp_before = gs.droids[0].hp;
    combat_tick(&creatures, &gs);
    ...
    assert(gs.droids[0].hp == hp_before);
    assert(gs.droids[0].shield_hp == 0);
    assert(memcmp(gs.droids[0].body_part_hp, body_part_hp,
                  sizeof(body_part_hp)) == 0);
    assert(creatures.last_attack_damage == 0);
    assert(creatures.attack_occurred);
}
...
int main(void) {
    ...
    test_combat_rejects_weapon_id_with_wrong_item_category();
    test_grenade_is_not_consumed_without_item_data();
    test_grenade_rejects_id_with_wrong_item_category();
    ...
    test_combat_shield_absorbs_before_body_damage();
