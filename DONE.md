...
- Resolved the long-open question "what fills the 64x32 map at DS:0x7CB3": the
  Captive DOS level is NOT generated by CAPPO.EXE (the front-end/menu) but by a
  separate child program, **GM.EXE**, which CAPPO launches via INT 21h AH=4Bh
  (EXEC) from its level-setup path (0xC9D9 -> 0x0DBC, program name at DGROUP:0x08
  = "GM.EXE"). This is why CAPPO alone never produced a level — with EXEC stubbed
  in the RE harness, GM.EXE never ran, so every state-1->3 transition bounced back
  to the menu.
- Reverse-engineered GM.EXE (LZEXE-unpacked, 40408b) end-to-end and ran it headless
  in the Unicorn harness on the real game files: it reads mission params from fixed
  low memory (0000:04F4), generates a 64x32 base via a placement state machine
  (0x1F00) + a 2048-cell generate/translate loop (0xEE), and writes the map to
  0x2CF:0x5A68. Verified it produces DISTINCT real base layouts per mission
  (ax=1/2/3 differ) with clear rooms and corridors.
- Transcribed GM.EXE's per-cell code translator (0x129) byte-for-byte into
  `src/data/captive_gm_translate.c` (`captive_gm_translate_cell`) and verified it in
  `tests/test_captive_gm_translate.c` against the full 256-entry ground-truth table
  captured from the real GM.EXE, plus the dh/ax/marker special cases. All 70 tests
  pass. Documented the full chain in `docs/CAPTIVE_DOS_DUNGEON_RE.md` and corrected
  the now-resolved note in `include/captive_dos_generator.h`.
- RE tooling preserved in `opencaptive-re/` (emu_drive2.py with INT 21 AH=4Bh EXEC,
  gmbuf.py/gmclean.py headless generation, xlate_ref.py ground-truth capture).
